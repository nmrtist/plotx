...
impl fmt::Display for OpenPathError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::OriginProbe(OriginError::UnsupportedVersion { raw_version }) => write!(
                formatter,
                "PlotX recognized classic Origin project version {raw_version}, but this build imports only the verified Origin 7.0552 and Origin 9.51 build 195 W64 OPJ profiles. No data was imported"
            ),
...
pub(crate) fn recent_open_kind(path: &Path) -> RecentOpenKind {
    if path.is_dir() {
        RecentOpenKind::Folder
    } else {
        extension_open_kind(path)
    }
}
...
pub(crate) fn dispatch_classified_path(
    ...
    match classified {
        ClassifiedOpenPath::Project => app.request_project_transition(
