...
use crate::ui::file_dialogs::recent::{
    OPEN_HEADER_BYTES, OpenPathEntryType, classify_open_path, classify_open_path_with_header,
};
#[cfg(unix)]
use crate::ui::file_dialogs::recent::{
    classify_open_handle, dispatch_classified_path, open_file_for_classification,
};
...
#[test]
fn unsupported_classic_opj_reports_its_version_and_supported_profile() {
    let path = temp_origin_file("opj", b"CPYA 4.3224 220 W64 #\n");
    let mut app = PlotxApp::new_with_settings(plotx_core::settings::Settings::default());
    ...
    let report = app
        ...
        .operations()
        .next_back()
        .expect("unsupported OPJ must produce a failure report");
    assert_eq!(report.outcome, OperationOutcome::Failure);
    assert!(report.summary.contains("4.3224 220 W64"), "{report:?}");
    assert!(report.summary.contains("Origin 7.0552"), "{report:?}");
    assert!(
        report.summary.contains("Origin 9.51 build 195 W64"),
        "{report:?}"
    );
    assert!(
        report.summary.contains("No data was imported"),
