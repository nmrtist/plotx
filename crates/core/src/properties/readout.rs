...
impl PlotxApp {
    ...
    ///
    /// Returns `None` when the target is not a contour series at all. A target
    /// that is one but whose estimate has not arrived returns a readout with
    /// [`ContourAnchor::Measuring`]: the difference between "no such setting"
    /// and "not measured yet" is one the interface has to be able to state.
    pub fn contour_base_readout(&self, target: &TargetRef) -> Option<ContourBaseReadout> {
        let context = self.series_context_unchecked(target).ok()?;
        ...
        let source = VersionedFieldRef {
            ...
            version: self.session.compute.current_field_version(FieldRef {
                resource: context.dataset.resource_id(),
                field: context.field,
            })?,
        };
        let summary = self.session.compute.peek_field_summary(source);
        let anchor = match &spec.positive.base {
            ...
            ContourBasePolicy::BackgroundScale { estimator, .. } => self.anchor_of(&EstimateKey {
                source,
        ...
        let lowest_level = summary.and_then(|summary| {
            match resolve_contour_levels(source, spec, summary, |key| {
                self.session.compute.peek_estimate(key).cloned()
            }) {
    ...
    ///
    /// Without a cached summary the floor cannot be compared against anything,
    /// and answering `Measured` would assert the estimate is in force when it
    /// may not be. A field with no cached summary has never been drawn, so
    /// reporting the measurement as still outstanding is both true and the only
    /// thing this read-only module may do about it.
    fn floored_anchor_of(
        ...
        let (Some(EstimateResult::Scale(estimate)), Some(summary)) =
            (self.session.compute.peek_estimate(key), summary)
        else {
    ...
    fn anchor_of(&self, key: &EstimateKey) -> ContourAnchor {
        let scale = match self.session.compute.peek_estimate(key) {
