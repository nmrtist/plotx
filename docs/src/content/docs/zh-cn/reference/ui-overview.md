...
- **主侧栏（Primary Side Bar）**——左侧面板。**Canvas** 模式列出图形、
  ...
  每个画布条目都带有实时页面缩略图；单击选择，双击名称重命名，右键打开上下文菜单。
- **画布（Canvas）/ 画板（board）**——中央区域：承载图形与普通数据表的
...
- **操作历史（Operation history）**——点击 Ribbon 任务行的时钟箭头图标，
  可查看当前状态和数据集摘要。**Messages** 保留本次运行最近 200 条已观察到的
  状态变化，**Diagnostics** 显示结构化操作报告；**Clear** 清空两个列表。
  悬停图标可查看当前状态，错误与警告也会出现在反馈横幅中。
...
- **图版面板（Panel）**——图版中带标签的分区，可以包含图表、图片、文本与形状。
  Panel 不能嵌套在其他 Panel 中。**编组（Group）**可让多个对象一起移动和排列，
  但不会添加 Panel 标签。
...
- **设置分组（settings group）**——一组同属一处的相关设置。Ribbon 为每个
  ...
  *Contour settings…*；<kbd>Ctrl</kbd>+<kbd>K</kbd> 则能找到分组里的单项
  ...
  打开。
- **数据表（Data sheet）**——数据表格的电子表格视图，双击表格打开。同步区域表
  保留在 Data 浏览器中并以只读方式打开；画板上只为它创建提取曲线图框架。需要
  与来源区域分离并可编辑的数值时，使用 **Save editable snapshot**。
