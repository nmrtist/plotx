...
| mzML | `.mzML` | 使用 32 位或 64 位数组的质心/轮廓谱图及 TIC、BPC、SIM 或 SRM 色谱图 |
...
要创建提取离子色谱图（XIC，也称 EIC），请打开 **Dataset tools → Mass
spectrometry** 并选择 **Select m/z range**。在当前质谱图上拖出 m/z 区间，
确认显示的 m/z 区间和采集流，然后选择 **Extract ion chromatogram**。

生成的折线图会在每次扫描的保留时间（分钟）处显示一个点。每个点的值是
所选 m/z 区间内的强度之和，并包含两个端点。该结果会被保存；移动扫描预览或
选择其他采集流都不会改变它。XIC 提取不进行色谱峰积分，也不计算峰面积。

数据中含多个受支持的 MS 功能时，请使用 **Dataset tools → Mass spectrometry**
中的 **Acquisition stream**。初始采集流是第一个非参考 MS 功能。切换采集流、
提取质谱和提取 XIC 都可通过标准的 Edit 命令撤销和重做。

PlotX 支持已使用 SQD2 数据验证的低分辨率 MassLynx 编码。如果必需的 MS 功能
使用其他编码，导入会停止并指出相应功能和仪器。如果其余数据可读，不受支持的
可选功能或参考功能会产生导入警告。

PlotX 不提供 LC–MS 处理流程。导入的数据、当前采集流、检测器通道、提取质谱、
提取离子色谱图和页面排版都会保存在 `.plotx` 项目中。扫描预览是临时状态，
重新打开项目后会被清除。
